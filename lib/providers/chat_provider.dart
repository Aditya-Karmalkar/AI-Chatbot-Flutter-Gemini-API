...
class ChatProvider extends ChangeNotifier {
  ...
  // current mode
  String _modelType = 'gemini-1.5-flash-001';
  ...
  Future<void> setModel({required bool isTextOnly}) async {
    if (isTextOnly) {
      _model = _textModel ??
          GenerativeModel(
            model: setCurrentModel(newModel: 'gemini-1.5-flash-001'),
            apiKey: ApiService.apiKey,
          );
    } else {
      _model = _visionModel ??
...
//?yeha samma
  ...
  Future<void> sentMessage({
    ...
    try {
      ...
      if (!ApiService.isValidApiKey()) {
        throw Exception(
            'Invalid API key format. Please check your API key configuration.');
      }
      ...
      log('Error in sentMessage: $error');
      // Create an error message in the chat
      final errorMessage = Message(
        messageId: DateTime.now().millisecondsSinceEpoch.toString(),
        chatId: currentChatId.isEmpty ? const Uuid().v4() : currentChatId,
        role: Role.assistant,
        message: StringBuffer('Error: ${error.toString()}'),
        imagesUrls: [],
        timeSent: DateTime.now(),
      );
  ...
  Future<void> sendMessageAndWaitForResponse({
    ...
    try {
      ...
      log('API Key being used: ${ApiService.apiKey}');
      log('Model type: $_modelType');
      log('Is text only: $isTextOnly');

      // start the chat session - only send history is its text-only
      final chatSession = _model!.startChat(
        history: history.isEmpty || !isTextOnly ? null : history,
      );
      log('Chat session created successfully');
      ...
      log('Content prepared successfully');
      ...
      log('Error in sendMessageAndWaitForResponse: $error');
      log('Stack trace: $stackTrace');
      setLoading(value: false);
    }
  }
